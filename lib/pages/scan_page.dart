...
class _ScanPageState extends State<ScanPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _initCamera() async {
    ...
    if (_cameras != null && _cameras!.isNotEmpty) {
      if (_isFirstLoad) {
        ...
        _isFirstLoad = false; // Toggle to false so user rotators work correctly later
      }
  ...
  // 🛡️ Helper Method to validate if an image file contains a human face
  Future<bool> _isValidFaceImage(File file) async {
    try {
      final inputImage = InputImage.fromFile(file);
      final List<Face> faces = await _faceDetector.processImage(inputImage);
      // return faces.isNotEmpty; // Returns true if a face is present
  ...
  // 📂 Updated: Validates gallery picks before displaying/stopping animations
  Future<void> _pickImage() async {
    final XFile? pickedFile = await _picker.pickImage(source: ImageSource.gallery);
    if (pickedFile != null) {
      File temporaryFile = File(pickedFile.path);
      
      setState(() => _loading = true); // Show loading while checking
      bool hasFace = await _isValidFaceImage(temporaryFile);
      setState(() => _loading = false);

      if (!hasFace) {
        // 🛑 Alert user and block the invalid image
        if (!mounted) return;
        _showInvalidImageDialog("The selected image does not appear to contain a clear view of a face. Please select a valid facial image.");
        return; 
      }

      setState(() {
        _imageFile = temporaryFile;
        _result = null;
        _scanController.stop(); 
      });
    }
  ...
  // 📸 Updated: Extra safety guard during live camera capture
  Future<void> _captureFromLiveFeed() async {
    if (_cameraController == null || !_cameraController!.value.isInitialized) return;
    
    // Safety Interceptor: Block capture if ML Kit doesn't see a face right now
    if (!_isFaceCentered) {
      _showInvalidImageDialog("Please position your face clearly inside the camera frame before capturing.");
      return;
    }
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child: Text("OK", style: TextStyle(color: colorPrimary, fontWeight: FontWeight.bold)),
          )
        ],
      ),
    );
